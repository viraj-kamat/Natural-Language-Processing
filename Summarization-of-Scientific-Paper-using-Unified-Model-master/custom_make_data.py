...
def chunk_all():
    ...
    for set_name in ['train', 'val', 'test']:
        print
        "Splitting %s data into chunks..." % set_name
        chunk_file(set_name)
    print
    "Saved chunked data in %s" % chunks_dir


def tokenize_stories(dir, prefix, tokenized_output_dir):
    """Maps a whole directory of .story files to a tokenized version using Stanford CoreNLP Tokenizer"""
    print
    "Preparing to tokenize %s to %s..." % (dir, tokenized_output_dir)
    abstracts = os.listdir(dir)
    # make IO list file
    print
    "Making list of files to tokenize..."
    with codecs.open("mapping.txt", "w", encoding="utf8") as f:
        for s in abstracts:
            f.write("%s \t %s\n" % (os.path.join(dir, s), os.path.join(tokenized_output_dir, s)))
    command = ['java', 'edu.stanford.nlp.process.PTBTokenizer', '-ioFileList', '-preserveLines', 'mapping.txt']
    print
    "Tokenizing %i files in %s and saving in %s..." % (len(abstracts), dir, tokenized_output_dir)
    ...
    num_orig = len(os.listdir(dir))
    num_tokenized = len(os.listdir(tokenized_output_dir))
    if num_orig != num_tokenized:
        raise Exception(
            "The tokenized stories directory %s contains %i files, but it should contain the same number as %s (which has %i files). Was there an error during tokenization?" % (
            tokenized_output_dir, num_tokenized, dir, num_orig))
    print
    "Successfully finished tokenizing %s to %s.\n" % (dir, tokenized_output_dir)


def read_text_file(text_file):
    lines = []
    with codecs.open(text_file, "r") as f:
        for line in f:
...
def write_to_bin(dir, tokenized_dir, out_file, makevocab=False):
    """Reads the tokenized .story files corresponding to the urls listed in the url_file and writes them to a out_file."""
    tokenized_abstracts = os.listdir(tokenized_dir)
    num_abstract = len(os.listdir(tokenized_dir))
    ...
    with codecs.open(out_file, 'wb') as writer:
        for idx, s in tqdm(enumerate(tokenized_abstracts)):
            if idx % 1000 == 0:
                print
                "Writing story %i of %i; %.2f percent done" % (
                idx, num_abstract, float(idx) * 100.0 / float(num_abstract))
    ...
    with codecs.open(out_file[:-4] + '_gt.pkl', 'wb') as out:
        pk.dump(data, out)
...
if __name__ == "__main__":
    ...
    dir = sys.argv[1]
    prefix = sys.argv[2]
    abstract_tokenized_dir = sys.argv[3]
    ...
    if not os.path.exists(abstract_tokenized_dir): os.makedirs(abstract_tokenized_dir)
    if not os.path.exists(finished_files_dir): os.makedirs(finished_files_dir)

    # Run stanford tokenizer on both stories dirs, outputting to tokenized stories directories
    tokenize_stories(dir, prefix, abstract_tokenized_dir)

    # Read the tokenized stories, do a little postprocessing then write to bin files
    write_to_bin(dir, abstract_tokenized_dir, os.path.join(finished_files_dir, "test.bin"), makevocab=True)

    with codecs.open(os.path.join(finished_files_dir, 'extract_info.pkl'), 'wb') as output_file:
