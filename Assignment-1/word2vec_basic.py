...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  while len(temp_labels) < batch_size :

    before = []
    after = []

    while len(temp_labels) < batch_size :
      skip_start = data_index - skip_window
      skip_end = data_index + skip_window

      while  skip_start < 0 and skip_start < data_index :
        skip_start += 1

      while skip_end > (len(data) - 1) and skip_end > data_index :
        skip_end -= 1

      if skip_start == data_index :
        before = []
      else :
        before = data[skip_start:data_index]

      if skip_end == data_index :
        after = []
      else :
        after = data[(data_index+1):(skip_end+1)]

      before.reverse()
      #after.reverse()
      count = 0

      while count < num_skips  :
        #if skip_start != data_index :
        if len(before) > 0 :
          batch.append(data[data_index])
          temp_labels.append(before.pop(0))
          skip_start += 1
          count += 1
        ...
        if len(temp_labels) == batch_size :
          break

        #if skip_end != data_index :
        if len(after) > 0 :
          batch.append(data[data_index])
          temp_labels.append(after.pop(0))
          skip_end -= 1
          count += 1


        if (count == num_skips) or (len(temp_labels) == batch_size) or (len(before) == 0 and len(after) == 0 ) :
          data_index += 1
          break
  ...
  for i in range (len(temp_labels)) :
    #labels = np.insert(labels,i,[temp_labels[i]])
    #print(labels[i])
...
def load_pretrained_model(sess, model, pretrained_model_path):
  ...
  print('Pretrained_path:='+str(pretrained_model_path))
...
if __name__ == '__main__':
  ...
  #virajk - uncomment the next two lines
  ...
  batch, labels = generate_batch(data, batch_size=12, num_skips=4, skip_window=8)
  for i in range(10):
