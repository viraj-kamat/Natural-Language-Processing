...
class ProbingClassifier(models.Model):
    def __init__(self,
        ...
        self.linear_layer = tf.keras.layers.Dense(units=classes_num,acitvation='softmax')
        # TODO(students): end
    ...
    def call(self, inputs: tf.Tensor, training: bool =False) -> tf.Tensor:
        ...
        # TODO(students): start
        try :
            data = self._pretrained_model(inputs)
            layer_representations = data["layer_representations"]
            layer_index = self._layer_num - 1
            last_layer = layer_representations[layer_index]
            logits = self.linear_layer(last_layer)
